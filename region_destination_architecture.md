# GlobeTrotter Region â†’ Destination â†’ Activity Architecture & Implementation Plan (Phase 3)
...
[City Table] (200 records)
     â”‚
     â””â”€â”€ 1:N â”€â”€> [Activities Table] (~796 records)
                     â”‚
                     â””â”€â”€ 1:N â”€â”€> [Trip Activities Table]
`

### Key Entities & Database Schema:
- cities: Stores basic location properties (id, 
ame, country, egion, cost_index, popularity, image_url, currency_code, currency_symbol). Note that egion is currently stored as a plain text VARCHAR(100) column.
- ctivities: Foreign key city_id references cities.id.
- 	rip_stops: Foreign key city_id references cities.id.
- 	rip_activities: Foreign key ctivity_id references ctivities.id.

---

## 2. Problems with the Current "City â†’ Activity" Model
...
2. **Lack of First-Class Region Concept**:
   - egion is just a string literal ('North India', 'South India', 'West India'). The system cannot support regional landing pages or regional trip planning (e.g., "Explore Kerala", "Explore Rajasthan").
...
[Countries] (Optional / System Constant)
    â”‚
    â””â”€â”€ 1:N â”€â”€> [Regions] (e.g., Kerala, Rajasthan, Ladakh, Western Ghats)
                    â”‚
                    â””â”€â”€ 1:N â”€â”€> [Destinations] (e.g., Jaipur, Munnar, Andaman Islands)
                                    â”œâ”€â”€ 1:N â”€â”€> [Destination Aliases] (e.g., Alleppey, Bangalore)
                                    â”‚
                                    â””â”€â”€ 1:N â”€â”€> [Activities / POIs] (e.g., Hawa Mahal, Tea Museum)
`
...
### B. Destination Entity (destinations table â€” Evolved from cities)
`java
@Entity
...
## 6. Evaluation: Option A vs Option B for City â†’ Destination Evolution
...
| **DB Foreign Keys** | Table rename (cities $\rightarrow$ destinations). FKs point to 1 place. | Dual FKs on 	rip_stops (city_id vs destination_id). Complex OR queries. |
...
2. **Spring Boot Controller Compatibility**:
   ...
   - CityResponse DTO remains identical in fields (id, 
ame, country, egion, imageUrl, etc.) so frontend TypeScript interfaces require zero immediate changes.
...
1. **Migration 1 (V12__create_regions_and_aliases_tables.sql)**:
   - Create egions table and seed 12 primary Indian travel regions (*Rajasthan, Kerala, Himalayas, Western Ghats, South India Temples, North India Circuit, East & Northeast, Central India, Goa & West Coast, Andaman & Nicobar, Lakshadweep, Islands*).
...
2. **Migration 2 (V13__evolve_cities_to_destinations.sql)**:
   ...
   - Rename column ctivities.city_id $\rightarrow$ destination_id (or add alias column).
   - Rename column 	rip_stops.city_id $\rightarrow$ destination_id.
3. **Migration 3 (V14__seed_137_curated_destinations_and_activities.sql)**:
   - Populate 137 target curated destinations, assign egion_ids, seed 9 key aliases (Bangalore, Alleppey, Pondicherry, Mysore, Vizag, Rameshwaram, Aurangabad, Kohlapur, Karjad).
...
| :--- | :---: | :--- |
| **Trip Stop FK Breakage** | HIGH | Update 	rip_stops FK constraints within the same Flyway transaction; preserve all existing destination IDs. |
