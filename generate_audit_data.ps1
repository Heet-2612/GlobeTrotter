...
Write-Host "Target Curated Count: $($targetCurated.Count)"
...
$mapping = @{
    ...
    "Bangalore" = @{ Target = "Bengaluru"; Status = "CURATED" }
    ...
    "Cherrapunji" = @{ Target = "Cherrapunji (Sohra)"; Status = "CURATED" }
    ...
    "Chikmagalur" = @{ Target = "Chikkamagaluru"; Status = "CURATED" }
    ...
    "Rameshwaram" = @{ Target = "Rameswaram"; Status = "CURATED" }
    "Tirupati" = @{ Target = "Tirupati"; Status = "CURATED" }
    "Vizag" = @{ Target = "Visakhapatnam"; Status = "CURATED" }
    ...
    "Aurangabad" = @{ Target = "Chhatrapati Sambhajinagar"; Status = "CURATED" }
...
foreach ($c in $cities) {
    $info = $mapping[$c.Name]
    if (-not $info) {
        Write-Host "Unmapped V8 City: $($c.Name)"
        continue
    }
    if ($info.Status -eq "CURATED") { $curatedFromV8 += $c }
    elseif ($info.Status -eq "MERGED") { $mergedFromV8 += $c }
    elseif ($info.Status -eq "SEARCH_ONLY") { $searchOnlyFromV8 += $c }
}

$curatedTargetsFromV8 = ($cities | ForEach-Object {
    $info = $mapping[$_.Name]
    if ($info.Status -eq "CURATED" -or $info.Status -eq "MERGED") {
        $info.Target
    }
}) | Select-Object -Unique

$newDestinations = @()
foreach ($target in $targetCurated) {
    if ($curatedTargetsFromV8 -notcontains $target) {
        $newDestinations += $target
    }
}

Write-Host "V8 Total Destinations: $($cities.Count)"
Write-Host "V8 Destinations mapped to CURATED: $($curatedFromV8.Count)"
Write-Host "V8 Destinations mapped to MERGED: $($mergedFromV8.Count)"
Write-Host "V8 Destinations mapped to SEARCH_ONLY: $($searchOnlyFromV8.Count)"
Write-Host "Unique Curated Destinations mapped from V8: $($curatedTargetsFromV8.Count)"
Write-Host "NEW Destinations to add to Catalog: $($newDestinations.Count)"

$totalReconciled = $curatedTargetsFromV8.Count + $newDestinations.Count
Write-Host "Total Target Catalog Reconciled: $totalReconciled (Target: $($targetCurated.Count))"

# Check math
Write-Host "`nReconciliation Verification Math:"
Write-Host "1. V8 Breakdown: 200 Existing = $($curatedFromV8.Count) (Curated) + $($mergedFromV8.Count) (Merged) + $($searchOnlyFromV8.Count) (Search-Only)"
Write-Host "2. Target Catalog: 137 Curated = $($curatedTargetsFromV8.Count) (from V8) + $($newDestinations.Count) (NEW)"
