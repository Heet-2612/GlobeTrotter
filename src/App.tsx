...
const MainAppContent: React.FC = () => {
  ...
  // Navigation tab state - default to login or shared link
  const [shareTokenParam, setShareTokenParam] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (window.location.pathname.startsWith('/shared/')) {
        return window.location.pathname.replace('/shared/', '');
      }
    }
    return null;
  });
  ...
  const [currentTab, setCurrentTab] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (window.location.hash.startsWith('#/share/')) return 'shared';
      if (window.location.pathname.startsWith('/shared/')) return 'shared';
    }
    return 'login';
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!shareToken && window.location.hash.startsWith('#/share/')) {
        shareToken = window.location.hash.replace('#/share/', '');
      }
      if (!shareToken && window.location.pathname.startsWith('/shared/')) {
        shareToken = window.location.pathname.replace('/shared/', '');
      }
      if (shareToken) {
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentTab === 'shared' && (
              <SharedItineraryPage
                shareToken={shareTokenParam || (typeof window !== 'undefined' ? (new URLSearchParams(window.location.search).get('share') || window.location.hash.replace('#/share/', '') || window.location.pathname.replace('/shared/', '')) : '')}
                onSelectTab={setCurrentTab}
              />
            )}
